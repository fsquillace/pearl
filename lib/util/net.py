...
__all__     = ('getIPs',  'IP',  'isIP',
               'getMACs', 'MAC', 'isMAC', 'get_ip_address', 'get_hw_address', 'get_netmask', 'convert_to_slash_notation')

import os, socket, struct, sys, fcntl
...
def get_ip_net_bdc(ip_str, mask_cidr):
    """
    Returns the network and broadcast ip starting from the ip and the mask
...
def get_hw_address(ifname):
    """
    Here is similar code to get the hardware address
    """
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
...
def get_netmask(ifname):
    """
    Return the netmask of the ifname
    """
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    return socket.inet_ntoa(fcntl.ioctl(s, 35099, struct.pack('256s', ifname))[20:24])

    # Altra versione
#def get_netmask2(ifname):
    #s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    #return socket.inet_ntoa(fcntl.ioctl(s.fileno(), 0x891b, struct.pack('256s',ifname))[20:24])

def convert_to_slash_notation(netmask):
    """
    Argument: netmask a string in X.X.X.X decimal format
    Return: netmask in /X notation
    """
    netmask = netmask.split('.')
    count=0

    for octect in netmask:
        m = (int)(octect)
        while(m!=0):
            if m%2==1:
                count+=1
...
#----------------------------------------------------------------------
def is_connect(host="www.google.it"):
    """
    Check the Internet Connection pinging the Google server (www.google.com).
    """
    st, out = getstatusoutput("ping "+host+" -c 1 >> /dev/null")
    return st==0
